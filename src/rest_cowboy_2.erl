...
rest_init(Req, _Opts) ->
    ...
    io:format("_17:~n\t~p~n_1701\t~p",[Resource,Req1]),
...
to_html(Req, #st{resource_module = M, resource_id = Id} = State) ->
    io:format("_36:~n\t~p",[Req]),
    Body = case Id of
               undefined -> [M:to_html(Resource) || Resource <- M:get_c(Req)];
               _ -> M:to_html(M:get_c(Req,Id)) end,
...
to_json(Req, #st{resource_module = M, resource_id = Id} = State) ->
    io:format("_48:~n\t~p",[Req]),
    Struct = case Id of
                 undefined -> [{M, [ M:to_json(Resource) || Resource <- M:get_c(Req) ] } ];
                 _         -> M:to_json(M:get_c(Req,Id)) end,
...
default_validate(Mod, Id, Data) ->
    %% io:format("_85:~n~p~n~p~n~p",[Mod,Id,Data]),
...
rest_module(Module) ->
    try M = list_to_existing_atom(Module),
        ...
        true = lists:member(rest_c, proplists:get_value(behaviour, Info)),
        {ok, M}
